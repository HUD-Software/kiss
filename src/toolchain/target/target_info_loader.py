...
class TargetInfoLoader:
    ...
    def _read_target_node(self, target_name: str, yaml_target : YamlObject) -> TargetInfo | None:
        ...
        if len(parts) != 4:
            console.print_error(f"Invalid target name '{target_name}' in '{self.file}({yaml_target.key_line})'")
        ...
        if not yaml_target.value:
            console.print_warning(f"'{target_name}' target is empty in '{self.file}({yaml_target.key_line})'")
            return 
            ...
            match item:
                case "arch":
                    ...
                    if not isinstance(yaml_object.value, str):
                        console.print_error(f"'arch' must be an arch name ( x86_64, i686, aarch64, arm ) in'{self.file}({yaml_target.key_line})'")
                        return None
                    target_file.arch = yaml_object.value
                case "vendor":
                    # Check that 'vendor' is a string
                    if not isinstance(yaml_object.value, str):
                        console.print_error(f"'vendor' must be a vendor ( pc, unknown ) name in'{self.file}({yaml_target.key_line})'")
                        return None
                    target_file.vendor = yaml_object.value
                case "os":
                    # Check that 'vendor' is a string
                    if not isinstance(yaml_object.value, str):
                        console.print_error(f"'os' must be an os name ( windows, linux, macos, none ) in'{self.file}({yaml_target.key_line})'")
                        return None
                    target_file.os = yaml_object.value
                case "abi":
                    # Check that 'abi' is a string
                    if not isinstance(yaml_object.value, str):
                        console.print_error(f"'abi' must be a abi name ( msvc, gnu, musl, none ) in'{self.file}({yaml_target.key_line})'")
                        return None
                    target_file.abi = yaml_object.value
                case "pointer-width":
                    # Check that 'pointer-width' is a string
                    if not isinstance(yaml_object.value, int):
                        console.print_error(f"'pointer-width' must be a integer in'{self.file}({yaml_target.key_line})'")
                        return None
                    target_file.pointer_width = yaml_object.value
                case "endianness":
                    # Check that 'endianness' is a string
                    if not isinstance(yaml_object.value, str):
                        console.print_error(f"'endianness' must be 'big' or 'little' in'{self.file}({yaml_target.key_line})'")
                        return None
                ...
                case "supported-compilers":
                    ...
                    if not isinstance(yaml_object.value, list) or not all(isinstance(x, str) for x in yaml_object.value):
                        console.print_error(f"'supported-compilers' must contains list of compiler name in '{self.file}'")
                        return None
                ...
                case "compilers":
                    ...
                    pass
                case _:
                    console.print_error(f"Unknown key '{item}' in '{self.file}({yaml_target.key_line})'")
                    continue
        return target_file
    ...
    def read_targets(self, item_yaml_object) -> bool:
        for target_name, target_yaml_object in item_yaml_object.value.items():
            ...
            if target_name in TargetRegistry:
                console.print_error(f"'{target_name}' target already exist when loading '{self.file}({target_yaml_object.key_line})'")
                continue
            
            # Load the target
            target_file = self._read_target_node(target_name, target_yaml_object)
            if target_file is None:
                console.print_error(f"'{target_name}' target in '{self.file}({target_yaml_object.key_line})' will not be available")
                continue
